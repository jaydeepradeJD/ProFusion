...
def get_grid_raybundle(cameras, image_size=(64, 64), max_depth=1.0,
...
def get_directional_raybundle(cameras, x_pos_ndc, y_pos_ndc, max_depth=1.0):
    ...
    xy_depth2 = torch.stack((x_pos_ndc, y_pos_ndc, torch.ones_like(x_pos_ndc) * 2.0), dim=-1)  # (N, 3)
    xy_depth = xy_depth.unsqueeze(0) # (1, N, 3)
    xy_depth2 = xy_depth2.unsqueeze(0) # (1, N, 3)
    num_points = xy_depth.shape[1]

    rays_depth1 = cameras.unproject_points(xy_depth.to(cameras.device), world_coordinates=True, from_ndc=True)  # (1, N, 3)
    rays_depth2 = cameras.unproject_points(xy_depth2.to(cameras.device), world_coordinates=True, from_ndc=True)  # (1, N, 3)
    directions = rays_depth2 - rays_depth1  # (1, N, 3)
    origins = rays_depth1  - directions  # (1, N, 3)

    # normalize directions
    directions = F.normalize(directions, dim=-1)  # (1, N, 3)

    origins = origins.to('cpu') # (1, N, 3)
    directions = directions.to('cpu') # (1, N, 3)
    # origins = origins.unsqueeze(0).to('cpu')  # (B, N, 3)
    # directions = directions.unsqueeze(0).to('cpu')  # (B, N, 3)
...
def get_grid_raybundle_gpu(
        cameras, image_size=(64, 64), max_depth=1.0,
        min_x=1, max_x=-1, min_y=1, max_y=-1
    ):
    if isinstance(image_size, int):
        image_width = image_size
...
def get_directional_raybundle_gpu(cameras, x_pos_ndc, y_pos_ndc, max_depth=1.0):
    ...
    xy_depth2 = xy_depth2.unsqueeze(0)
    num_points = xy_depth.shape[1]
    ...
    # normalize directions
    directions = F.normalize(directions, dim=-1)  # (1, N, 3)
    # origins = origins.unsqueeze(0)  # (B, N, 3)
    # directions = directions.unsqueeze(0)  # (B, N, 3)
